...
#
# A .sdkmod is a zip whose root contains exactly one folder, named the same as
# the file, with __init__.py inside it. Getting that structure wrong is the
...
OUTPUT = ROOT / "dist" / "bl2speed.sdkmod"
...
def main() -> None:
    ...
    with zipfile.ZipFile(OUTPUT, "w", zipfile.ZIP_DEFLATED) as archive:
        for path in sorted(MOD_FOLDER.rglob("*")):
            relative = path.relative_to(MOD_FOLDER.parent)
